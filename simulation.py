...
MARS_RADIUS = 3390000  # radius of Mars in meters
...
# LANDMASS_OUTLINE_COLOR = (255 / 255, 125 / 255, 20 / 125)  # black, best contrast
# MARS_LAND_OPACITY = 1.0
MARS_TEXTURE_PATH = "2k_mars.jpg"

MARS_BASE_COLOR = (255 / 255, 125 / 255, 20 / 125)  # red, color of Mars (base color)
MARS_OPACITY = 1.0

BACKGROUND_COLOR = (1.0, 1.0, 1.0)  # white

# SAT_COLOR = (0x22 / 0xFF, 0x88 / 0xFF, 0x33 / 0xFF)
SAT_COLOR = (
    0x14 / 0xFF,
    0x99 / 0xFF,
    0xFF / 0xFF,
...
SAT_OPACITY = 1.0
SAT_POINT_SIZE = 11  # how big satellites are in (probably) screen pixels
...
ISL_LINK_COLOR = (
    ...
    0x99 / 0xFF,
    0xFF / 0xFF,
)  # yellow-brown, satellite-satellite links
ISL_LINK_OPACITY = 0.6
ISL_LINE_WIDTH = 2  # how wide to draw line in pixels
...
MARS_SPHERE_POINTS = 5000  # higher = smoother earth model, slower to generate

# SECONDS_PER_DAY = 86400  # number of seconds per earth rotation (day)
SECONDS_PER_DAY = 88775.244  # number of seconds per mars rotation (day)
...
class Simulation:
    def __init__(
        ...
        if gmlImportFileName is not None:
            ...
            self.min_communications_altitude = int(
                G.graph["minCommunicationsAltitudeMeters"]
            )
            ...
            if self.make_links:
                ...
                self.model.calculatePlusGridLinks(
                    self.max_stg_distance, max_isl_range=self.max_isl_distance
                )
    ...
    def initializeNetworkDesign(self):
        ...
        if self.linking_method == "IDEAL":
            self.model.calculateIdealLinks(self.max_isl_distance, self.max_stg_distance)

        if self.linking_method == "+GRID":
            self.model.calculatePlusGridLinks(
                self.max_stg_distance, initialize=True, crosslink_interpolation=1
            )
    ...
    def updateModel(self, new_time):
        ...
        if self.make_links:
            if self.linking_method == "IDEAL":
                self.model.calculateIdealLinks(
                    self.max_isl_distance, self.max_stg_distance
                )
            if self.linking_method == "+GRID":
                self.model.calculatePlusGridLinks(
                    self.max_stg_distance, max_isl_range=self.max_isl_distance
        ...
        if (
            ...
            or self.enable_path_calculation
        ):
        ...
        self.time_to_export_gml = time.time() - self.time_1 - self.time_to_update_model
    ...
    def setupAnimation(
        ...
        self.makeMarsActors(MARS_RADIUS)

        self.total_groundpoints = total_groundpoints

        if total_satellites > 0:
            self.makeSatsActor(total_satellites, satellite_positions)

        if self.total_groundpoints > 0:
    ...
    def updateAnimation(self, obj, event):
        ...
        if not self.pause:
            ...
            # rotate mars and land
            rotation_per_time_step = 360.0 / (SECONDS_PER_DAY / self.time_step)
            # self.marsActor.RotateZ(rotation_per_time_step)
        ...
        # update gnd pts
        if self.total_groundpoints > 0:
            for i in range(self.total_groundpoints):
        ...
        if not self.pause:
            self.time_to_export_img = (time.time() - self.time_1) - (
                self.time_to_update_model
                + self.time_to_export_gml
    ...
    def makeRenderWindow(self):
        ...
        # self.renderer.AddActor(self.marsActor)
        if self.total_groundpoints > 0:
    ...
    def makeGndPtsActor(self, total_groundpoints, groundpoint_positions):
        ...
        # init a array for IDs ?
        self.gndPointIDs = [None] * self.total_groundpoints
    ...
    def makeMarsActors(self, mars_radius):
        """
        generate the mars sphere, and the landmass outline

        Parameters
        ----------
        mars_radius : int
                radius of the Mars in meters

        """

        self.marsRadius = mars_radius

        # Create mars map
        # a point cloud that outlines all the earths landmass
        # self.marsSource = vtk.vtkEarthSource()
        # draws as an outline of landmass, rather than fill it in
        # self.marsSource.OutlineOn()
        ...
        # Create an actor
        # self.marsActor = vtk.vtkActor()
        # self.marsActor.SetMapper(self.marsMapper)

        # set color
        # self.marsActor.GetProperty().SetColor(LANDMASS_OUTLINE_COLOR)
        ...
        # load mars texture
        self.marsTexture = vtk.vtkJPEGReader()
        self.marsTexture.SetFileName(MARS_TEXTURE_PATH)

        # create texture map
        self.marsTextureMap = vtk.vtkTexture()
        self.marsTextureMap.SetInputConnection(self.marsTexture.GetOutputPort())

        # make sphere data
        num_pts = MARS_SPHERE_POINTS
        indices = np.arange(0, num_pts, dtype=float) + 0.5
        phi = np.arccos(1 - 2 * indices / num_pts)
        theta = np.pi * (1 + 5**0.5) * indices
        x = np.cos(theta) * np.sin(phi) * self.marsRadius
        y = np.sin(theta) * np.sin(phi) * self.marsRadius
        z = np.cos(phi) * self.marsRadius
        ...
        map_to_sphere.SetInputConnection(dss.GetOutputPort())
        ...
        self.sphereActor.SetTexture(self.marsTextureMap)

        # set color
        # self.sphereActor.GetProperty().SetColor(MARS_BASE_COLOR)
        # self.sphereActor.GetProperty().SetOpacity(MARS_OPACITY)
