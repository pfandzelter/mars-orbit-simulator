...
)  # distance of the link in meters
...
class Constellation:
    ...
    def __init__(
        ...
        self.groundpoints_array = np.zeros(NUM_GROUND_POINTS, dtype=GROUNDPOINT_DTYPE)
        ...
        self.raan_offsets = [
            (arcOfAscendingNodes / self.number_of_planes) * i
    ...
    def setConstillationTime(self, time=0.0):
        ...
        # rotation_matrix = self.getRotationMatrix(EARTH_ROTATION_AXIS, degrees_to_rotate)
        rotation_matrix = self.getRotationMatrix(MARS_ROTATION_AXIS, degrees_to_rotate)
    ...
    def generateNetworkGraph(self, city_names):
        ...
        for gnd_idx in range((-self.ground_node_counter)):
            self.G.add_node(
                str(self.groundpoints_array[gnd_idx]["ID"]),
    ...
    def addGroundPoint(self, latitude, longitude, altitude=100.0):
        ...
        if self.current_time == 0 or self.current_time % SECONDS_PER_DAY == 0:
            ...
            rotation_matrix = self.getRotationMatrix(
    ...
    def calculateMaxISLDistance(self, min_communication_altitude):
        ...
        c = MARS_RADIUS + min_communication_altitude
            ...
            for sat_idx_a in range(self.total_sats - 1):
                ...
                for sat_idx_b in range(sat_idx_a + 1, self.total_sats):
                    ...
                    d = int(
                        math.sqrt(
                            math.pow(
                                self.satellites_array[sat_idx_b]["x"] - sat_a_pos[0], 2
                            )
                            + math.pow(
                                self.satellites_array[sat_idx_b]["y"] - sat_a_pos[1], 2
                            )
                            + math.pow(
                                self.satellites_array[sat_idx_b]["z"] - sat_a_pos[2], 2
                            )
                        )
                    )
            ...
            for gnd_idx in range(-self.ground_node_counter):
                ...
                for sat_idx in range(self.total_sats):
                    ...
                    d = int(
                        math.sqrt(
                            math.pow(
                                self.satellites_array[sat_idx]["x"] - gnd_pos[0], 2
                            )
                            + math.pow(
                                self.satellites_array[sat_idx]["y"] - gnd_pos[1], 2
                            )
                            + math.pow(
                                self.satellites_array[sat_idx]["z"] - gnd_pos[2], 2
                            )
                        )
                    )
                    # deicide if link is valid or not
        ...
        if USING_NUMBA is True:
            ...
            for isl_idx in range(self.number_of_isl_links):
                ...
                d = int(
                    math.sqrt(
                        math.pow(
                            self.satellites_array[sat_1]["x"]
                            - self.satellites_array[sat_2]["x"],
                            2,
                        )
                        + math.pow(
                            self.satellites_array[sat_1]["y"]
                            - self.satellites_array[sat_2]["y"],
                            2,
                        )
                        + math.pow(
                            self.satellites_array[sat_1]["z"]
                            - self.satellites_array[sat_2]["z"],
                            2,
                        )
                    )
                )
